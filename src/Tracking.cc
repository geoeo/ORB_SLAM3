...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const CameraParameters &cam, const ImuParameters &imu, const OrbParameters &orb, const int sensor):
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr)
{
  ...
  mpORBextractorLeft = new ORBextractor(orb.nFeatures, orb.scaleFactor, orb.nLevels, orb.iniThFast, orb.minThFast);

  if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
    mpORBextractorRight = new ORBextractor(orb.nFeatures, orb.scaleFactor, orb.nLevels, orb.iniThFast, orb.minThFast);

  if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
    mpIniORBextractor = new ORBextractor(5*orb.nFeatures, orb.scaleFactor, orb.nLevels, orb.iniThFast, orb.minThFast);

  cout << endl << "ORB Extractor Parameters: " << endl;
  cout << "- Number of Features: " << orb.nFeatures << endl;
  cout << "- Scale Levels: " << orb.nLevels << endl;
  cout << "- Scale Factor: " << orb.scaleFactor << endl;
  cout << "- Initial Fast Threshold: " << orb.iniThFast << endl;
  cout << "- Minimum Fast Threshold: " << orb.minThFast << endl;
  ...
  {
    const float sf = sqrt(imu.freq);

    mpImuCalib = new IMU::Calib(imu.Tbc, imu.noiseGyro*sf, imu.noiseAccel*sf, imu.gyroWalk/sf, imu.accelWalk/sf);

    mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib);

    cout << endl;
    cout << "IMU frequency: " << imu.freq << " Hz" << endl;
    cout << "IMU gyro noise: " << imu.noiseGyro << " rad/s/sqrt(Hz)" << endl;
    cout << "IMU gyro walk: " << imu.gyroWalk << " rad/s^2/sqrt(Hz)" << endl;
    cout << "IMU accelerometer noise: " << imu.noiseAccel << " m/s^2/sqrt(Hz)" << endl;
    cout << "IMU accelerometer walk: " << imu.accelWalk << " m/s^3/sqrt(Hz)" << endl;
