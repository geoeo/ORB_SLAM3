...
int main(int argc, char **argv) {
  ...
  std::cout << "Opening file: " << std::string("ORBvoc.txt") << std::endl;
  std::cout << "Saving to file: " << std::string("ORBvoc.bin") << std::endl;

  load_as_text(voc, std::string("ORBvoc.txt"));
  save_as_binary(voc, std::string("ORBvoc.bin"));
